...
const handlers = {
    ...
    async sendMainMenu(userNumber, userInfo, isInitial = true) {
        ...
        let menuOptions = 'Digite uma opção:\n1 - Boletos\n2 - Prestação de contas\n3 - Datas de entrega dos boletos\n4 - Datas de entrega das prestações de contas\n\nPara sair digite "sair" ou "s"';
...
// Exibe as datas de entrega dos boletos para o ano de 2025, lendo as informações do arquivo 'agenda_boletos.json' na pasta dados.
    ...
    // Nova função para exibir as datas de entrega das prestações de contas
    async handleDatasContas(userNumber, userInfo) {
        try {
            const contasFilePath = criarCaminhoDados('agenda_prestacao_contas.json');
            const contasData = JSON.parse(fs.readFileSync(contasFilePath, 'utf8'));
            const datasContas2025 = contasData['2025'];

            let message = "**Datas de entrega das prestações de contas em 2025:**\n\n";
            for (const mes in datasContas2025) {
                message += `${mes}: ${datasContas2025[mes]}\n`;
            }
            ...
            await client.sendMessage(userNumber, optionsMessage);
            logInteraction(userNumber, userInfo, optionsMessage, 'system_message');
            stateManager.states[userNumber] = 'datas_contas_menu';

        } catch (error) {
            logInteraction(userNumber, userInfo, `Erro ao acessar datas das prestações de contas: ${error.message}`, 'error');
            await client.sendMessage(userNumber, "Erro ao acessar as datas de entrega das prestações de contas.");
            logInteraction(userNumber, userInfo, 'Erro ao acessar as datas de entrega das prestações de contas', 'system_message');
        }
    },
    ...
    switch (stateManager.states[userNumber]) {
        case 'main_menu':
            if (normalizedBody === '1') {
                ...
                await handlers.handleDatasContas(userNumber, userInfo);
            } else {
                const invalidMessage = `Opção inválida: "${msg.body}". Escolha 1 para Boletos, 2 para Prestação de contas, 3 para Datas de entrega dos boletos, 4 para Datas de entrega das prestações de contas ou "s" para sair.`;
        ...
        case 'boletos_menu':
        case 'contas_menu':
        case 'datas_boletos_menu':
        case 'datas_contas_menu': // Adicionado o novo estado
            if (normalizedBody === '0') {
