    ...
    switch (stateManager.states[userNumber]) {
        case 'main_menu':
            if (normalizedBody === '1') {
                ...
                const seuDinheiroPath = path.join(__dirname, 'seu_dinheiro', 'seu_dinheiro_1.pdf');
                const filename = 'seu_dinheiro_1.pdf';
    
                try {
                    const media = MessageMedia.fromFilePath(seuDinheiroPath);
                    await client.sendMessage(userNumber, media, {
                        sendMediaAsDocument: true,
                        filename
                    });
                    ...
                    await client.sendMessage(userNumber, optionsMessage);
                    logInteraction(userNumber, userInfo, optionsMessage, 'system_message');
                    stateManager.states[userNumber] = 'seu_dinheiro_menu';
                } catch (error) {
                    logInteraction(userNumber, userInfo, `Erro ao enviar ${filename}: ${error.message}`, 'error');
                    await client.sendMessage(userNumber, `Erro ao enviar o arquivo Seu dinheiro. Verifique se o arquivo ${filename} existe na pasta 'seu_dinheiro'.`);
                    logInteraction(userNumber, userInfo, `Erro ao enviar o arquivo Seu dinheiro. Verifique se o arquivo ${filename} existe na pasta 'seu_dinheiro'.`, 'system_message');
                }
            } else {
