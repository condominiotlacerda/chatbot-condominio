...
const handlers = {
    ...
    async sendMainMenu(userNumber, userInfo, isInitial = true) {
      ...
      async handleBoletos(userNumber, chat, userInfo) {
        ...
        if (mesAtualIndex > 0) {
            const dataVencimentoMesAnterior = new Date(vencimentos2025[Object.keys(vencimentos2025)[mesAtualIndex - 1]].split('/').reverse().join('-'));
            const dataEntregaMesAtual = new Date(agenda2025[mesAtual].split('/').reverse().join('-'));
    
            const diaSeguinteVencimento = new Date(dataVencimentoMesAnterior);
            diaSeguinteVencimento.setDate(dataVencimentoMesAnterior.getDate() + 1);
    
            const diaAnteriorEntrega = new Date(dataEntregaMesAtual);
            diaAnteriorEntrega.setDate(dataEntregaMesAtual.getDate() - 1);
    
            if (hoje >= diaSeguinteVencimento && hoje <= diaAnteriorEntrega) {
                const proximaEntrega = agenda2025[mesAtual];
                const mensagem = `Não há boletos do mês anterior. Próxima entrega: ${proximaEntrega}`;
                await client.sendMessage(userNumber, mensagem);
                logInteraction(userNumber, userInfo, mensagem, 'system_message');
            } else {
                // Lógica para enviar os boletos individuais (fora do período)
...
setInterval(() => {
    substituirBoletosCondominio(client, CONFIG, path, fs, criarCaminhoDados, logInteraction, stateManager);
